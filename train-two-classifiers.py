...
def execute(rank,
    ...
    modelA = eval(architecture)(True)
    modelB = eval(architecture)(False)
    modelA = nn.SyncBatchNorm.convert_sync_batchnorm(modelA).to(rank)
    modelB = nn.SyncBatchNorm.convert_sync_batchnorm(modelB).to(rank)
    if rank == 0:
        logger.debug('Model architecture:\n{}'.format(str(modelA)))

    cudnn.benchmark = True
    netA = DDP(modelA, device_ids=[rank])
    netB = DDP(modelB, device_ids=[rank])
    ...
    schedulerA = lr_scheduler.MultiStepLR(
        optimizerA,
    ...
    schedulerB = lr_scheduler.MultiStepLR(
        optimizerB,
        milestones=np.arange(10, 50, 10),
        gamma=0.5
    )
    ...
    for epoch in range(1, training_pref['max_epochs']+1):
        ...
        for images, targets, tracks, spher in train_loader:
            ...
            lA = criterion(outputsA, targets)
            lB = criterion(outputsB, targets)

            pos = targets == 0
            corr = distance_corr(
                F.softmax(outputsA, dim=1)[:, 1][pos],
                F.softmax(outputsB, dim=1)[:, 1][pos],
                1
            )
            if torch.isnan(corr):
                corr = 0
        ...
        with torch.no_grad():
            ...
            for images, targets, tracks, spher in val_loader:
                outputsA = netA(images)
                outputsB = netB(images)
                predsA = torch.argmax(outputsA, dim=1)
                targets = torch.cuda.LongTensor(targets, device=rank)
                ...
                a = (targets == predsA).sum() / batch_size_validation
                acc.update(a.data)
        ...
        if rank == 0 and cp_es(v.sum(0), modelA, modelB):
            flag_tensor += 1
